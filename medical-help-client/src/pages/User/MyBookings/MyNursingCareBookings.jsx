...
const MyNursingCareBookings = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      fetch(
        `https://medical-help-server.vercel.app/nursing-bookings?email=${user.email}`,
      )
        .then((res) => res.json())
        .then((data) => {
          setBookings(data);
      ...
      {bookings.length > 0 ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-500 max-w-sm mx-auto">
            আপনি এখনও কোনো নার্সিং কেয়ার সার্ভিসের জন্য বুকিং করেননি।
          </p>
          <Link to={'/nursing-care-services'} className="btn btn-info mt-4 text-white">বুকিং করুন</Link>
        </div>
      )}
    </div>
  );
